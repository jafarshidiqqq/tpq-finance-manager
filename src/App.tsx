...
export default function App() {
  ...
  const handleAiInput = async () => {
    ...
    try {
      ...
      if (!apiKey) {
        throw new Error("Gemini API Key tidak ditemukan. Jika Anda menjalankan ini di luar AI Studio (seperti di Vercel), Anda harus menambahkan GEMINI_API_KEY ke Environment Variables di pengaturan proyek Anda. Anda bisa mendapatkan API Key di aistudio.google.com.");
      }
  ...
  const login = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/unauthorized-domain') {
        message = "Domain ini tidak diizinkan. Tambahkan domain Vercel Anda di Firebase Console > Authentication > Settings > Authorized domains.";
      } else if (error.code === 'auth/popup-closed-by-user') {
        message = "Jendela login tertutup. Pastikan Anda tidak menutupnya dan izinkan popup di browser Anda.";
      } else if (error.code === 'auth/cancelled-popup-request') {
        message = "Permintaan login dibatalkan karena ada permintaan baru.";
      ...
      <div className="min-h-screen bg-[#F8F9FA] flex flex-col items-center justify-center p-6 sm:p-10">
        ...
        >
          ...
          <button 
            ...
            className="w-full bg-white border border-gray-200 text-gray-700 py-3 sm:py-4 rounded-xl font-bold hover:bg-gray-50 transition-all flex items-center justify-center gap-3 disabled:opacity-50"
          >
            {isGoogleLoading ? (
